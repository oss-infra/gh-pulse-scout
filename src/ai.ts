...
function buildStatsBlock(result: AnalysisResult): string {
    ...
    const labels = result.topLabels.map((l) => `- ${l.name}: ${l.count}`).join('\n') || '- (none)';
    const hot =
        result.topHotIssues
            .map((i) => `- #${i.number} "${i.title}" (${i.comments} comments) ${i.url}`)
            .join('\n') || '- (none)';
...
export async function generateAiReport(
    ...
    if (!opts.apiKey) {
        throw new Error('Missing OPENAI_API_KEY. Set it in env or pass via .env file.');
    }
